...
> Bundled verbatim from the upstream `llms-full.txt`
> (https://raw.githubusercontent.com/coollabsio/coolify-cli/main/llms-full.txt).
> To refresh: `curl -fsSL https://raw.githubusercontent.com/coollabsio/coolify-cli/main/llms-full.txt -o references/command-catalog.md` and re-add this header.
>
> Every `coolify` subcommand, its flags, defaults, and which are required. Grep for
> a command (e.g. `app create public`, `database backup create`) to get its exact flags.
